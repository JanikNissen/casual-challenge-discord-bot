...
import {IsScryfallCardLink} from './utils.js';
...
client.on(Events.MessageCreate, async message => {
  if(message.author.id === process.env.SCRYFALL_DISCORD_BOT_ID){
    if(IsScryfallCardLink(message.content)){
      const scry = await ScryfallCardRequest(message.content.split('/').pop().replaceAll('-', '+'));
      await message.reply({embeds: [await getCardLegalityEmbed(scry.name)]});
    }
  }
});
